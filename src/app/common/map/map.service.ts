...
export class MapService {
    ...
    private getcodeLocation(location: string): Observable<any> {

        this.geoCored = new (<any>window).google.maps.Geocoder();

        if (!this.geoCored) {
            this.geoCored = new (<any>window).google.maps.Geocoder();
        }
        return new Observable((observer) => {
            this.geoCored.geocode({ address: location }, (result, status) => {
                if (status = 'OK') {
